...
/* Save the list to a file
    ...
    note:   The tasks are not necessarily stored in the file in
			priority order.
*/
...
void printList(DynArr *heap) {
    ...
    // Copy the main list to a temp list so
    // that tasks can be printed out and removed.
    ...
    while(sizeDynArr(temp) > 0) {
        ...
        /* Print the Task */
        printf("%d:  %s\n\n", task->priority, task->description);
        /* Remove the task, but let's not free up the memory it's pointing to since the old DynArr is using it!*/

...
void _dynArrSetCapacity(DynArr *v, int newCap) {
	...
	oldData = v->data;
	printf("========Resizing========\n");
    /* Create a New DynArr with Larger Underlying Array */
...
void removeAtDynArr(DynArr *v, int idx) {
    ...
    // I don't have to worry about copying an element outside the array into that idx!
...
void removeDynArr(DynArr *v, TYPE val, comparator compare) {
    ...
    assert(containsDynArr(v,val, compare));
    for(i = 0; i < sizeDynArr(v); i++)
        if((*compare)(v->data[i], val) == 0) {
            removeAtDynArr(v,i);
            break;
...
int containsDynArrOrd(DynArr *v, TYPE val, comparator compare){
    int idx = _binarySearch(v, val, compare);
    // REM: If it's not in there, it'll return size.
    // This could hold a previously added value...so must check the index!
...
void addHeap(DynArr *heap, TYPE val, comparator compare) {
    ...
    while(child != 0) {
        parent = (child - 1) / 2;
        if(compare(getDynArr(heap, child), getDynArr(heap, parent)) < 0) {
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare) {
    ...
    if(rightPos <= max) {
        least = _smallerIndexHeap(heap, leftPos, rightPos, compare);
        if(compare(getDynArr(heap, least), getDynArr(heap, pos)) == -1) {
            swapDynArr(heap, pos, least);
            _adjustHeap(heap, max, least, compare);
        }
    }
    else if(leftPos <= max) {
        if(compare(getDynArr(heap, leftPos), getDynArr(heap, pos)) == -1) {
            swapDynArr(heap, pos, leftPos);
            _adjustHeap(heap, max, leftPos, compare);
        }
    }
}
...
void _buildHeap(DynArr *heap, comparator compare) {
    ...
    for(i = heap->size - 1; i >= 0; i--) {
        _adjustHeap(heap, heap->size - 1, i, compare);
    }
}
